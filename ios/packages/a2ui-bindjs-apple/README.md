...
host.onAction = { action in
    // the agent's half of the conversation
}
...
That is the whole surface area. `setValue`, `reset`, `surfaceIds`, `diagnostics`,
`ast(for:)`, and `useCatalog` round it out; nothing else needs to be public.

## Your own components

Register your own BindJS components on the host before the first surface arrives:
...
the rest still comes from the bundle. The sources are the same strings the web renderer
takes as `sources`, so one component restyles both.
[`examples/ios/custom-catalog`](../../../examples/ios/custom-catalog/README.md) and
`examples/web/custom-catalog` share theirs verbatim.
...
components and agent-authored surfaces share a runtime, and interaction works in both. A
renderer carrying a second runtime would draw a correct-looking tree and then do nothing
when tapped.
...
store through `a2ui.onChange` and publishes on that instead. This is what `useA2UIStore`
does on the web with `useSyncExternalStore`. Get it wrong and the store updates correctly
while the screen keeps showing the tree it drew first.
...
### Why not inject a `JSContext`

The obvious alternative is for `BindJSContext` to accept a context from outside, so the
renderer owns it. `BindJSContext.init` installs an exception handler, timers, `console`,
the `runtime` global and the callbacks hung off it, and it assumes it is alone in there.
Accepting a foreign context means two instances can land in one `JSContext` and silently
overwrite each other's `runtime`: the single-runtime failure above, moved somewhere much
harder to see. And it would not remove the second member: decoding an AST runs through
`toDirective`, `makeComponent`, `resolveForEachChildren`, and `ComponentView`, none of
which are reachable from JavaScript. Exposing the context is the same capability with none
of that: the host adds to a namespace the runtime still owns.
...
paths. Handing `view` the builder is what makes that ordering impossible to get wrong,
rather than a rule in a doc comment.

## Resources

`Sources/A2UI/Resources/a2ui-native.js` is the renderer, built from the npm package by
`pnpm sync:native` and committed. It is versioned in lockstep with that package: it must
agree with the runtime about AST shape, so the two are not independently upgradable.

## Not implemented yet

`callRendererFunction` and `agentFunctionResponse` cross the bridge but have no host-facing
API, so an agent that calls back into the renderer has nowhere to land.
