...
//
// Identical to the iOS example's, message for message and source for source: `FlightCard`
// is one A2UI type standing in for a whole row of chrome — logo, route, times, status dot,
// price and a book action. BindJS draws it as Compose here and as SwiftUI there.
//
// The tap animation runs on BindJS `useState` and timers, entirely inside the component.
// The agent never learns about it, and the host does not have to declare it.
...
object Flights {
    ...
    val sources: Map<String, String> by lazy { mapOf("FlightCard" to flightCard) }

    /**
     * A2UI type → BindJS component name, merged over the basic catalog. `Column` and
     * `Text` on this surface are still the bundled ones.
     */
    val catalog: Map<String, String> = mapOf("FlightCard" to "FlightCard")
     ...
     *
     * The template's paths are relative to the row, and its action context is resolved per
     * row — so the third card's tap arrives carrying `AA 184`. A `List` would be the other
     * way to write these rows and is avoided on purpose: it is a `ScrollView`, and a
     * surface that scrolls itself clips its content inside a scrolling screen.
     */
    val messages = """
    [
      { "version": "v1.0",
    ...
    /**
     * One card, not templated, for the instrumented test.
     *
     * Template rows are built lazily inside the `ForEach` callback, so they are not in the
     * tree `render` returns. This one is, which is what makes its contents checkable.
     */
    val singleCard = """
    [
      { "version": "v1.0",
    ...
    /**
     * Props arrive under their A2UI names. `action` is the engine's: it carries the event
     * name and the context it resolved for this row, so the component only has to call it.
     */
    private val flightCard = """
