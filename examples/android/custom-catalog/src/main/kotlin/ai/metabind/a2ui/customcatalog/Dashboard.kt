...
object Dashboard {
    ...
    val sources: Map<String, String> by lazy { mapOf("A2UIChart" to chart) }
     ...
     */
    val catalog: Map<String, String> = mapOf("Chart" to "A2UIChart")

    // MARK: - The agent's messages

    /**
     * One surface: a heading, a total, and three charts over three lists in the data model.
     *
     * `formatCurrency` is an A2UI standard function, so the total is formatted by the engine
     * before the component sees it — the chart formats its own axis and readout, because those
     * are drawn inside a component the engine cannot reach into. The surface names no catalog
     * id, so it renders through the one the host was given.
     */
    val messages = """
    [
      { "version": "v1.0",
