...
Android: BindJS components the app wrote, registered on the host, and named by the agent as
...
`ScrollView`, and a surface that scrolls itself has to be told the host scrolls too —
...
`CustomCatalogTest.kt` proves all of it without a screen. It
is an instrumented test, so it needs a device or emulator; CI builds the app but cannot run
it.
