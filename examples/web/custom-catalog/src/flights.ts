 ...
 *
 * Identical to the iOS and Android examples', message for message and source for source:
 * `FlightCard` is one A2UI type standing in for a whole row of chrome — logo, route,
 * times, status dot, price and a book action. BindJS draws it as DOM here and as SwiftUI
 * or Compose there.
 *
...
export const FLIGHT_SOURCES = { FlightCard: FLIGHT_CARD }

export const FLIGHT_CATALOG: Catalog = { ...BASIC_CATALOG, FlightCard: 'FlightCard' }

// ─── The agent's messages ────────────────────────────────────────────────────
//
// One surface: a heading, a subheading, and a `Column` templated over `/flights`. The
// template's paths are relative to the row, and its action context is resolved per row —
// so the third card's tap arrives carrying `AA 184`.
//
// A `List` would be the other way to write these rows and is avoided on purpose: it is a
// `ScrollView`, and a surface that scrolls itself clips its content inside a scrolling
// host.

export const FLIGHT_MESSAGES: AgentMessage[] = [
      { "version": "v1.0",
        "createSurface": {
          "surfaceId": "flights",
          "components": [
