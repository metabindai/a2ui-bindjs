...
export const HABITAT_SOURCES = { A2UISortBoard: SORT_BOARD }

/**
 * A2UI type → BindJS component, merged over the basic catalog. Everything else on this
 * surface is still the bundled component.
 */
export const HABITAT_CATALOG: Catalog = { ...BASIC_CATALOG, SortBoard: 'A2UISortBoard' }

// ─── The agent's messages ────────────────────────────────────────────────────
//
// One surface: a heading and the board, over nine animals and three habitats.
//
// `answer` travels on each card rather than in a separate key, so it survives the
// component's shuffle. The surface names no catalog id, so it renders through the
// one the host was given.

export const HABITAT_MESSAGES: AgentMessage[] = [
  { "version": "v1.0",
    "createSurface": {
      "surfaceId": "habitat",
      "components": [
