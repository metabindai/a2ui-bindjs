...
Copy `.env.example` to `.env.local` and fill in your organisation, project and package ids,
plus either an API key or a preview token. Without them the example runs on the bundled
catalog, so you can see it working first.
...
In dev, requests are proxied through Vite. In production, proxy from your own server: the
Metabind API sends no CORS headers, and an API key in a browser bundle is not a secret.
...
- **Text bound to data** can arrive as a number, so convert it (`String(props.text)`) before
  rendering it. Left alone it takes out the whole surface, not just that one label.

## The same package on iOS

BindJS components render as real SwiftUI, so a package written for the web restyles the
native app as well — see `examples/ios/minimal`. Hand the same components to the native host
...
Reach for something with no native equivalent and it will look right in the browser and
