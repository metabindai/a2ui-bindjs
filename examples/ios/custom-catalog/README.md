...
of it without a window. `ContentView.swift` is the index and the chrome; no part of it
describes an offer, a star, a flight, or a chart.
...
both platforms and the same source restyles the web app too. Stick to the color names it
knows as well, since an unknown one produces no color rather than an error.
